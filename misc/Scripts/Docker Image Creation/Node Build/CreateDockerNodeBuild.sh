...
DEBIAN_FRONTEND=noninteractive chroot $rootfs apt-get -y install apt-transport-https ca-certificates
...
chroot $rootfs apt-get update
DEBIAN_FRONTEND=noninteractive chroot $rootfs apt-get -y install ssh wget unzip binutils debhelper devscripts automake autoconf libtool sqlite3 libsqlite3-dev libncurses5-dev libssl-dev libparse-debcontrol-perl libgpg-error-dev libxslt1-dev libedit-dev libenchant-dev libqdbm-dev libcrypto++-dev libltdl-dev zlib1g-dev libtinfo-dev libgmp-dev libxml2-dev libzip-dev p7zip-full ntp libavahi-common-dev libavahi-client-dev
...
if [[ -z "$NODEBUILD_SERVERNAME" || -z "$NODEBUILD_SERVERPORT" || -z "$NODEBUILD_SERVERUSER" || -z "$NODEBUILD_REPOSITORYSERVERPATH" || -z "$NODEBUILD_NIGHTLYSERVERPATH" || -z "$NODEBUILD_SERVERCERT" ]]; then
	...
	mkdir -p /root/.ssh
	echo "$NODEBUILD_SERVERCERT" > /root/.ssh/id_ed25519
