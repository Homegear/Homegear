...
echo "deb http://homegear.eu/packages/Debian/ stretch/
...
if test -f homegear.deb; then
	isodate=`date +%Y%m%d`
EOF
echo "	mv homegear.deb homegear_\$[isodate]_debian_stretch_$arch.deb
	/build/Upload.sh
...
docker build -t homegear/build:debian-stretch-$arch "$dir"
