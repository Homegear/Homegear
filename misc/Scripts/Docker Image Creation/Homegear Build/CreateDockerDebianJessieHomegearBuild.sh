...
print_usage() {
	echo "Usage: CreateDockerDebianJessiePHPBuild.sh ARCH"
...
debootstrap --no-check-gpg --foreign --arch=$arch jessie $rootfs http://ftp.debian.org/debian
if [ "$arch" == "armhf" ]; then
...
echo "deb http://homegear.eu/packages/Debian/ jessie/
...
chroot $rootfs apt-get -y install ssh unzip ca-certificates binutils debhelper devscripts sqlite3 libsqlite3-dev libreadline6 libreadline6-dev libncurses5-dev libssl-dev libparse-debcontrol-perl libgcrypt20-dev libgpg-error-dev libgnutls28-dev php5-dev libphp5-embed libssl-dev libcrypto++-dev
...
if test -f homegear.deb; then
	isodate=`date +%Y%m%d`
EOF
echo "	mv homegear.deb homegear_\$[isodate]_debian_jessie_$arch.deb
	/build/Upload.sh
...
docker build -t homegear/build:debian-jessie-$arch "$dir"
