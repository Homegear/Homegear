...
echo "deb http://homegear.eu/packages/Raspbian/ stretch/
...
if test -f homegear.deb; then
	isodate=`date +%Y%m%d`
EOF
echo "	mv homegear.deb homegear_\$[isodate]_raspbian_stretch_$arch.deb
	/build/Upload.sh
...
docker build -t homegear/build:raspbian-stretch "$dir"
