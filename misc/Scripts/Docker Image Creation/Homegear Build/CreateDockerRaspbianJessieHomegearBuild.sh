...
echo "deb http://homegear.eu/packages/Raspbian/ jessie/
...
if test -f homegear.deb; then
	isodate=`date +%Y%m%d`
EOF
echo "	mv homegear.deb homegear_\$[isodate]_raspbian_jessie_$arch.deb
	/build/Upload.sh
...
docker build -t homegear/build:raspbian-jessie "$dir"
