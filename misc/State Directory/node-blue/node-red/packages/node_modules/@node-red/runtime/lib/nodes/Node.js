...
var events = require("../events"); //Added by Homegear GmbH to emit node output events.
...
//{{{ Implemented by Homegear GmbH to send node output to Homegear
const NOOP_SEND = function(msg) {
    try {
        for (i = 0; i < msg.length; i++) {
            if(util.isArray(msg[i])) {
                for (j = 0; j < msg[i].length; j++) {
                    delete msg[i][j].inputIndex;
                }
            }
            else delete msg[i].inputIndex;
        }
        this.homegear.invoke("noderedEvent", ["global", "node-output", {nodeId: this.id, message: msg, synchronous: this.synchronousOutput}]);
    } catch (e) {
        console.log(e);
    }
}
...
function Node(n) {
    ...
    //{{{ Methods added by Homegear GmbH
    this.getNodeData = function(key, value) {
        try {
            return this.homegear.invoke("getNodeData", [this.id, key]);
        } catch (e) {
            console.log(e);
        }
        return null;
    ...
    this.setNodeData = function(key, value) {
        try {
            this.homegear.invoke("setNodeData", [this.id, key, value]);
        } catch (e) {
            console.log(e);
        }
    }
...
Node.prototype.warn = function(msg) {
    this.homegear.invoke("noderedEvent", ["global", "error", {nodeId: this.id, level: Log.WARN, message: {
        error: {
            "message": msg,
            "source": {
                z: this.z,
...
Node.prototype.error = function(logMessage,msg) {
    ...
    if (!handled) {
        this.homegear.invoke("noderedEvent", ["global", "error", {nodeId: this.id, level: Log.ERROR, message: {
            error: {
                "message": logMessage,
                "source": {
                    z: this.z,
