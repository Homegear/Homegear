...
class Flow {
    ...
    constructor(parent,globalFlow,flow) {
        ...
        events.on("homegear-event",this.homegearEvent.bind(this)); //Added by Homegear GmbH to listen for node input events.
    }
    ...
    handleStatus(node,statusMessage,reportingNode,muteStatusEvent) {
        ...
        if (!muteStatusEvent) {
            //{{{ Added by Homegear GmbH - add fields needed by Homegear
    ...
    //Added by Homegear GmbH to listen for node input events.
    homegearEvent(event) {
        if (event.id != 'node-input') return;
        if (this.activeNodes.hasOwnProperty(event.payload.nodeId)) {
            if (!event.payload.message.hasOwnProperty('_msgid')) event.payload.message._msgid = redUtil.generateId();
            if (!event.payload.message.hasOwnProperty('topic')) event.payload.message.topic = '';
            let activeNode = this.activeNodes[event.payload.nodeId];
            activeNode.synchronousOutput = event.payload.synchronous;
            activeNode.receive(event.payload.message);
        }
    }
