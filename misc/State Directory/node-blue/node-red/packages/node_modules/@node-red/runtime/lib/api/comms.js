...
function handleStatusEvent(event) {
    if (!event.status) {
        ...
        var status = {
            ...
            source: event.status.source //Added by Homegear GmbH - add required fields for Homegear
        };
...
function publish(topic,data,retain) {
    if (topic == "debug") return; //Added by Homegear GmbH - debug messages are handled in file Node.js
    if (retain) {
        //retained[topic] = data; //Deleted by Homegear GmbH - unneeded as retained by Homegear
    } else {
        delete retained[topic];
    }
    try {
        runtime.homegear.get().invoke("noderedEvent", ["global", "frontend-event", {topic: topic, data: data, retain: retain}]); //Added by Homegear GmbH to send event to frontend.
    } catch (e) {
        console.log(e);
    }
