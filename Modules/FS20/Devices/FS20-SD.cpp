...
namespace FS20
{
FS20_SD::FS20_SD(IDeviceEventSink* eventHandler) : FS20Device(eventHandler)
{
	init();
}

FS20_SD::FS20_SD(uint32_t deviceID, std::string serialNumber, int32_t address, IDeviceEventSink* eventHandler) : FS20Device(deviceID, serialNumber, address, eventHandler)
{
	init();
}
...
void FS20_SD::init()
{
	try
	{
		...
		_deviceType = (uint32_t)DeviceType::FS20SD;
	}
...
void FS20_SD::saveVariables()
{
	try
	{
		if(_deviceID == 0) return;
		FS20Device::saveVariables();
...
void FS20_SD::loadVariables()
{
	try
	{
		FS20Device::loadVariables();
		_databaseMutex.lock();
...
void FS20_SD::serializeFilters(std::vector<uint8_t>& encodedData)
{
	try
	{
		BaseLib::BinaryEncoder encoder;
		encoder.encodeInteger(encodedData, _filters.size());
		for(std::list<FS20_SD_Filter>::const_iterator i = _filters.begin(); i != _filters.end(); ++i)
		{
...
void FS20_SD::unserializeFilters(std::shared_ptr<std::vector<char>> serializedData)
{
	try
	{
...
bool FS20_SD::onPacketReceived(std::string& senderID, std::shared_ptr<BaseLib::Systems::Packet> packet)
{
	try
	{
		if(!_enabled) return false;
		std::shared_ptr<FS20Packet> fs20Packet(std::dynamic_pointer_cast<FS20Packet>(packet));
		if(!fs20Packet) return false;
		...
		for(std::list<FS20_SD_Filter>::const_iterator i = _filters.begin(); i != _filters.end(); ++i)
		{
			switch(i->filterType)
			{
				case FilterType::SenderAddress:
					if(fs20Packet->senderAddress() == i->filterValue) printPacket = true;
					break;
				case FilterType::DestinationAddress:
					if(fs20Packet->destinationAddress() == i->filterValue) printPacket = true;
					break;
			}
		}
		if(_filters.size() == 0) printPacket = true;
		if(printPacket) std::cout << BaseLib::HelperFunctions::getTimeString(fs20Packet->timeReceived()) << " FS20 packet received: " + fs20Packet->hexString() << std::endl;
	}
    catch(const std::exception& ex)
    {
    	BaseLib::Output::printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
void FS20_SD::addFilter(FilterType filterType, int32_t filterValue)
{
	try
	{
		FS20_SD_Filter filter;
...
void FS20_SD::removeFilter(FilterType filterType, int32_t filterValue)
{
	try
	{
		_filters.remove_if([&](FS20_SD_Filter filter){ return filter.filterType == filterType && filter.filterValue == filterValue; });
	}
    catch(const std::exception& ex)
    {
    	BaseLib::Output::printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
std::string FS20_SD::handleCLICommand(std::string command)
{
	try
	{
		...
		if(command == "help")
		{
			...
			stringStream << "send\t\t\tSends a FS20 packet" << std::endl;
		...
		if(command.compare(0, 12, "filters list") == 0)
		{
			...
			while(std::getline(stream, element, ' '))
			{
				...
				else if(index == 2)
				{
					if(element == "help")
					{
						stringStream << "Description: This command lists all FS20 packet filters." << std::endl;
		...
		else if(command.compare(0, 11, "filters add") == 0)
		{
			...
			if(index < 4)
			{
				stringStream << "Description: This command adds a FS20 packet filter. All filters are linked by \"or\". Currently there is no option to connect the filters by \"and\"" << std::endl;
		...
		else if(command.compare(0, 14, "filters remove") == 0)
		{
			...
			if(index < 4)
			{
				stringStream << "Description: This command removes a FS20 packet filter." << std::endl;
		...
		else if(command.compare(0, 4, "send") == 0)
		{
			...
			if(index == 1)
			{
				stringStream << "Description: Sends a FS20 packet." << std::endl;
			...
			std::shared_ptr<FS20Packet> packet;
