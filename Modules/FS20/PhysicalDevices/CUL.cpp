...
CUL::CUL(std::shared_ptr<BaseLib::Systems::PhysicalInterfaceSettings> settings) : BaseLib::Systems::IPhysicalInterface(settings)
{
}
...
void CUL::openDevice()
{
	try
	{
		...
		if(lockfileDescriptor == -1)
		{
			...
			{
				BaseLib::Output::printCritical("CUL device is in use: " + _settings->device);
				return;
			}
...
std::string CUL::readFromDevice()
{
	try
	{
		...
		if(_fileDescriptor->descriptor == -1)
		{
			BaseLib::Output::printCritical("Couldn't read from CUL device, because the file descriptor is not valid: " + _settings->device + ". Trying to reopen...");
			closeDevice();
		...
		while((!_stopCallbackThread && localBuffer[0] != '\n'))
		{
			...
			switch(i)
			{
				...
				case -1:
					BaseLib::Output::printError("Error reading from CUL device: " + _settings->device);
					return "";
				case 1:
					break;
				default:
			...
			if(packet.size() > 200)
			{
				BaseLib::Output::printError("CUL was disconnected.");
				closeDevice();
				return "";
			}
		}
...
void CUL::writeToDevice(std::string data, bool printSending)
{
    try
    {
        ...
        {
            BaseLib::Output::printInfo("Info: Sending: " + data.substr(2, data.size() - 4));
        }
        _sendMutex.lock();
