...
void FS20Packet::import(std::vector<uint8_t>& packet)
{
	try
	{
		...
		{
			BaseLib::Output::printWarning("Warning: Tried to import FS20 packet of wrong size (" + std::to_string(packet.size()) + " bytes).");
			return;
		}
		_packet = packet;
...
void FS20Packet::import(std::string packetHex, bool removeFirstCharacter)
{
	try
	{
		...
		{
			BaseLib::Output::printWarning("Warning: Tried to import FS20 packet larger than 128 bytes.");
			return;
		}
