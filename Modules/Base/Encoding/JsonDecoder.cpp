...
{
	variable->type = VariableType::rpcStruct;
	...
	while(pos < json.length())
	{
		...
		if(json[pos] != ':')
		{
			variable->structValue->insert(RPCStructElement(name, std::shared_ptr<Variable>(new Variable(VariableType::rpcVoid))));
			if(json[pos] == ',')
			{
				pos++;
				skipWhitespace(json, pos);
		...
		skipWhitespace(json, pos);
		if(!posValid(json, pos)) throw JsonDecoderException("No closing '}' found.");
		std::shared_ptr<Variable> element(new Variable(VariableType::rpcVoid));
		decodeValue(json, pos,element);
		variable->structValue->insert(RPCStructElement(name, element));
		skipWhitespace(json, pos);
		if(!posValid(json, pos)) throw JsonDecoderException("No closing '}' found.");
		if(json[pos] == ',')
		{
...
{
	variable->type = VariableType::rpcArray;
	if(!posValid(json, pos)) return;
	if(json[pos] == '[')
	{
		pos++;
...
{
	value->type = VariableType::rpcVoid;
	pos += 4;
}
