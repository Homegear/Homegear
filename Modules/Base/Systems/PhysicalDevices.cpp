		...
		while (fgets(input, 1024, fin))
		{
			...
			if(input[0] == '[')
			{
				while(ptr < len)
				{
					...
					{
						input[ptr] = '\0';
						if((!settings->device.empty() || (!settings->hostname.empty() && !settings->port.empty())) && !settings->type.empty() && settings->family != DeviceFamilies::none && Obj::ins->deviceFamilies.find(settings->family) != Obj::ins->deviceFamilies.end())
						{
							std::shared_ptr<PhysicalDevice> device = Obj::ins->deviceFamilies.at(settings->family)->createPhysicalDevice(settings);
							_physicalDevicesMutex.lock();
						...
						if(Obj::ins->deviceFamilies.find(settings->family) != Obj::ins->deviceFamilies.end()) Output::printDebug("Debug: Reading config for physical device family " + Obj::ins->deviceFamilies.at(settings->family)->getName());
						else Output::printError("Error in physicaldevices.conf: No module found for device family: " + name);
						break;
					}
					ptr++;
				}
			...
			if(found && Obj::ins->deviceFamilies.find(settings->family) != Obj::ins->deviceFamilies.end())
			{
				...
				if(name == "devicetype")
				{
					...
					settings->type = value;
					Output::printDebug("Debug: deviceType of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + settings->type);
				}
				else if(name == "device")
				{
					settings->device = value;
					Output::printDebug("Debug: device of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + settings->device);
				}
				else if(name == "responsedelay")
				{
					...
					Output::printDebug("Debug: responseDelay of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->responseDelay));
				}
				else if(name == "oneway")
				{
					HelperFunctions::toLower(value);
					if(value == "true") settings->oneWay = true;
					Output::printDebug("Debug: oneWay of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->oneWay));
				}
				else if(name == "enablerxvalue")
				{
					int32_t number = HelperFunctions::getNumber(value);
					settings->enableRXValue = number;
					Output::printDebug("Debug: enableRXValue of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->enableRXValue));
				}
				else if(name == "enabletxvalue")
				{
					int32_t number = HelperFunctions::getNumber(value);
					settings->enableTXValue = number;
					Output::printDebug("Debug: enableTXValue of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->enableTXValue));
				}
				else if(name == "gpio1")
				{
					int32_t number = HelperFunctions::getNumber(value);
					if(number > 0)
					{
						...
						Output::printDebug("Debug: GPIO1 of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->gpio[1].number));
					}
				}
				else if(name == "gpio2")
				{
					...
					if(number > 0)
					{
						...
						Output::printDebug("Debug: GPIO2 of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->gpio[2].number));
					}
				}
				else if(name == "gpio3")
				{
					...
					if(number > 0)
					{
						...
						Output::printDebug("Debug: GPIO3 of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->gpio[3].number));
					}
				}
				else if(name == "hostname")
				{
					settings->hostname = value;
					Output::printDebug("Debug: Hostname of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + settings->hostname);
				}
				else if(name == "port")
				{
					settings->port = value;
					Output::printDebug("Debug: Port of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + settings->port);
				}
				else if(name == "ssl")
				{
					HelperFunctions::toLower(value);
					if(value == "true") settings->ssl = true;
					Output::printDebug("Debug: SSL of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->ssl));
				}
				else if(name == "verifycertificate")
				{
					HelperFunctions::toLower(value);
					if(value == "false") settings->verifyCertificate = false;
					Output::printDebug("Debug: VerifyCertificate of family " + Obj::ins->deviceFamilies.at(settings->family)->getName() + " set to " + std::to_string(settings->verifyCertificate));
				}
				else
				{
...
void PhysicalDevices::setup(int32_t userID, int32_t groupID)
{
	try
	{
		...
		for(std::map<DeviceFamilies, std::shared_ptr<PhysicalDevice>>::iterator i = _physicalDevices.begin(); i != _physicalDevices.end(); ++i)
		{
			if(!i->second)
			{
				Output::printCritical("Critical: Could not setup device for device family with index " + std::to_string((int32_t)i->first) + " device pointer was empty.");
				continue;
			}
			Output::printDebug("Debug: Setting up physical device for device family with index " + std::to_string((int32_t)i->first));
...
std::shared_ptr<RPC::RPCVariable> PhysicalDevices::listInterfaces()
{
	try
	{
		...
		for(std::map<DeviceFamilies, std::shared_ptr<DeviceFamily>>::iterator i = Obj::ins->deviceFamilies.begin(); i != Obj::ins->deviceFamilies.end(); ++i)
		{
