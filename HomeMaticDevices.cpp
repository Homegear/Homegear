...
void HomeMaticDevices::initializeDatabase()
{
	try
	{
		...
		GD::db.executeCommand("CREATE TABLE IF NOT EXISTS events (eventID INTEGER PRIMARY KEY UNIQUE, name TEXT NOT NULL, type INTEGER NOT NULL, address TEXT, variable TEXT, trigger INTEGER, eventMethod TEXT, eventMethodParameters BLOB, resetAfter INTEGER, initialTime INTEGER, operation INTEGER, incrementBy DOUBLE, maxTime INTEGER, resetAfter INTEGER, resetMethod TEXT, resetMethodParameters BLOB, eventTime INTEGER, recurAfter INTEGER)");
