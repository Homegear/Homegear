...
namespace Insteon
{
Insteon_SD::Insteon_SD() : InsteonDevice()
{
	init();
}

Insteon_SD::Insteon_SD(uint32_t deviceID, std::string serialNumber, int32_t address) : InsteonDevice(deviceID, serialNumber, address)
{
	init();
}
...
bool Insteon_SD::packetReceived(std::shared_ptr<Packet> packet)
{
	try
	{
		...
		std::cout << GD::helperFunctions->getTimeString(insteonPacket->timeReceived()) << " Insteon packet received: " + insteonPacket->hexString() << std::endl;
	}
    catch(const std::exception& ex)
    {
    	GD::output->printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
