...
namespace HMWired
{
HMWiredMessage::HMWiredMessage()
{
}

HMWiredMessage::HMWiredMessage(int32_t messageType, HMWiredDevice* device, int32_t access, void (HMWiredDevice::*messageHandler)(std::shared_ptr<HMWiredPacket>)) : _messageType(messageType), _device(device), _access(access), _messageHandlerIncoming(messageHandler)
{
    _direction = DIRECTIONIN;
}

HMWiredMessage::HMWiredMessage(int32_t messageType, HMWiredDevice* device, int32_t access, int32_t accessPairing, void (HMWiredDevice::*messageHandler)(std::shared_ptr<HMWiredPacket>)) : _messageType(messageType), _device(device), _access(access), _accessPairing(accessPairing), _messageHandlerIncoming(messageHandler)
{
    _direction = DIRECTIONIN;
}

HMWiredMessage::HMWiredMessage(int32_t messageType, int32_t controlByte, HMWiredDevice* device, void (HMWiredDevice::*messageHandler)(std::shared_ptr<HMWiredPacket>)) : _messageType(messageType), _controlByte(controlByte), _device(device), _messageHandlerOutgoing(messageHandler)
{
...
void HMWiredMessage::invokeMessageHandlerIncoming(std::shared_ptr<HMWiredPacket> packet)
{
	try
	{
...
void HMWiredMessage::invokeMessageHandlerOutgoing(std::shared_ptr<HMWiredPacket> packet)
{
	try
	{
		if(_device == nullptr || _messageHandlerOutgoing == nullptr || !packet) return;
		((_device)->*(_messageHandlerOutgoing))(packet);
	}
	catch(const std::exception& ex)
	{
		Output::printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
bool HMWiredMessage::typeIsEqual(int32_t messageType, std::vector<std::pair<uint32_t, int32_t> >* subtypes)
{
	try
	{
...
bool HMWiredMessage::typeIsEqual(std::shared_ptr<HMWiredPacket> packet)
{
	try
	{
...
bool HMWiredMessage::typeIsEqual(std::shared_ptr<HMWiredMessage> message)
{
	try
	{
...
bool HMWiredMessage::typeIsEqual(std::shared_ptr<HMWiredMessage> message, std::shared_ptr<HMWiredPacket> packet)
{
	try
	{
...
bool HMWiredMessage::checkAccess(std::shared_ptr<HMWiredPacket> packet, std::shared_ptr<HMWiredQueue> queue)
{
	try
	{
		...
		if((access & ACCESSUNPAIRING) && queue != nullptr && queue->getQueueType() == HMWiredQueueType::UNPAIRING)
		{
			return true;
		}
		if(access & ACCESSPAIREDTOSENDER)
		{
			std::shared_ptr<HMWiredPeer> currentPeer;
