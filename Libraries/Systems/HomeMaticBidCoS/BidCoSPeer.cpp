...
void BidCoSPeer::initializeCentralConfig()
{
	try
	{
		...
		BaseLib::db.executeCommand("SAVEPOINT bidCoSPeerConfig" + std::to_string(_address));
    ...
    BaseLib::db.executeCommand("RELEASE bidCoSPeerConfig" + std::to_string(_address));
}
	...
	try
	{
		for(std::vector<std::shared_ptr<BasicPeer>>::iterator i = _peers[channel].begin(); i != _peers[channel].end(); ++i)
		{
			if((*i)->address == address && (*i)->channel == remoteChannel)
			{
				...
				BaseLib::db.executeCommand("DELETE FROM parameters WHERE peerID=? AND parameterSetType=? AND peerChannel=? AND remotePeer=? AND remoteChannel=?", data);
		...
		for(std::vector<FrameValues>::iterator a = frameValues.begin(); a != frameValues.end(); ++a)
		{
			...
			for(std::map<std::string, FrameValue>::iterator i = a->values.begin(); i != a->values.end(); ++i)
			{
				for(std::list<uint32_t>::const_iterator j = a->paramsetChannels.begin(); j != a->paramsetChannels.end(); ++j)
				{
					...
					saveParameter(parameter->databaseID, parameter->data);
					if(BaseLib::debugLevel >= 4) Output::printInfo("Info: " + i->first + " of HomeMatic BidCoS peer " + std::to_string(_peerID) + " with serial number " + _serialNumber + ":" + std::to_string(*j) + " was set to 0x" + HelperFunctions::getHexString(i->second.value) + ".");
