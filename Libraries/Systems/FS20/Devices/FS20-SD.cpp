...
namespace FS20
{
FS20_SD::FS20_SD() : FS20Device()
{
	init();
}

FS20_SD::FS20_SD(uint32_t deviceID, std::string serialNumber, int32_t address) : FS20Device(deviceID, serialNumber, address)
{
	init();
}
...
bool FS20_SD::packetReceived(std::shared_ptr<Packet> packet)
{
	try
	{
		...
		if(printPacket) std::cout << GD::helperFunctions->getTimeString(fs20Packet->timeReceived()) << " FS20 packet received: " + fs20Packet->hexString() << std::endl;
	}
    catch(const std::exception& ex)
    {
    	GD::output->printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
