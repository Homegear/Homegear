...
print_usage() {
	echo "Usage: CreateDockerDebianWheezyPHPBuild.sh ARCH"
...
debootstrap --no-check-gpg --foreign --arch=$arch wheezy $rootfs http://ftp.debian.org/debian
if [ "$arch" == "armhf" ]; then
...
docker build -t homegear/phpbuild:debian-wheezy-$arch "$dir"
