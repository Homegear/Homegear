...
print_usage() {
	echo "Usage: CreateDockerUbuntuUtopicHomegearBuild.sh ARCH"
...
echo "deb http://homegear.eu/packages/Ubuntu/ utopic/
...
if test -f homegear.deb; then
	isodate=`date +%Y%m%d`
EOF
echo "	mv homegear.deb homegear_\$[isodate]_ubuntu_utopic_$arch.deb
	/build/Upload.sh
...
docker build -t homegear/build:ubuntu-utopic-$arch "$dir"
