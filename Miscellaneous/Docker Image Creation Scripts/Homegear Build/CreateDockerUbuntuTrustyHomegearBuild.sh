...
print_usage() {
	echo "Usage: CreateDockerUbuntuTrustyHomegearBuild.sh ARCH"
...
echo "deb http://homegear.eu/packages/Ubuntu/ trusty/
...
if test -f homegear.deb; then
	isodate=`date +%Y%m%d`
EOF
echo "	mv homegear.deb homegear_\$[isodate]_ubuntu_trusty_$arch.deb
	/build/Upload.sh
...
docker build -t homegear/build:ubuntu-trusty-$arch "$dir"
