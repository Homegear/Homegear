...
print_usage() {
	echo "Usage: CreateDockerUbuntuVividHomegearBuild.sh ARCH"
...
echo "deb http://homegear.eu/packages/Ubuntu/ vivid/
...
if test -f homegear.deb; then
	isodate=`date +%Y%m%d`
EOF
echo "	mv homegear.deb homegear_\$[isodate]_ubuntu_vivid_$arch.deb
	/build/Upload.sh
...
docker build -t homegear/build:ubuntu-vivid-$arch "$dir"
