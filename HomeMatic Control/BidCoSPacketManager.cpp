...
void BidCoSPacketManager::set(int32_t address, std::shared_ptr<BidCoSPacket>& packet)
{
	try
	{
		if(_packets.find(address) != _packets.end() && _packets[address] && _packets[address]->thread) _packets[address]->stopThread = true;
...
void BidCoSPacketManager::deletePacket(int32_t address, uint32_t id)
{
	try
	{
		...
		while(_packets.find(address) != _packets.end() && _packets[address] && !_packets[address]->stopThread && std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() <= _packets[address]->time + 1000)
		{
