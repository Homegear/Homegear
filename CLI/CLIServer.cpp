...
Server::Server()
{
	_socketDir = "/var/run/Homegear/";
	_socketPath = _socketDir + "Homegear.sock";
}
...
void Server::getFileDescriptor()
{
	...
	{
		struct stat sb;
		if(stat(_socketDir.c_str(), &sb) == -1)
		{
			if(errno == ENOENT && GD::debugLevel >= 1) std::cerr << "Directory " << _socketDir << " does not exist. Please create it before starting Homegear." << std::endl;
			else throw Exception("Error reading information of directory " + _socketDir + ": " + strerror(errno));
			return;
		}
		if(!S_ISDIR(sb.st_mode))
		{
			if(GD::debugLevel >= 1) std::cerr << "Directory " << _socketDir << " does not exist. Please create it before starting Homegear." << std::endl;
			return;
		}
		if(unlink(_socketPath.c_str()) == -1 && errno != ENOENT) throw(Exception("Couldn't delete existing socket: " + _socketPath + ". Error: " + strerror(errno)));
		int32_t fileDescriptor;
		if((fileDescriptor = socket(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0)) == -1) throw(Exception("Couldn't create socket: " + _socketPath + ". Error: " + strerror(errno)));
		sockaddr_un serverAddress;
		serverAddress.sun_family = AF_UNIX;
		strcpy(serverAddress.sun_path, _socketPath.c_str());
		...
		chmod(_socketPath.c_str(), S_IRWXU | S_IRGRP | S_IXGRP);
    }
    catch(const std::exception& ex)
    {
