...
void Client::systemListMethods(std::pair<std::string, std::string>& address)
{
	try
	{
		...
		{
			if(server->removed || (server->getServerClientInfo()->sendEventsToRpcServer && server->getServerClientInfo()->closed) || (server->socket && !server->socket->connected() && server->keepAlive && !server->reconnectInfinitely)) return;
...
void Client::collectGarbage()
{
	try
	{
		...
		{
			if(i->second->removed || (i->second->getServerClientInfo()->sendEventsToRpcServer && (i->second->getServerClientInfo()->closed || !i->second->getServerClientInfo()->socket->connected())) || (i->second->socket && !i->second->socket->connected() && i->second->keepAlive && !i->second->reconnectInfinitely) || (!i->second->initialized && now - i->second->creationTime > 120))
			{
...
BaseLib::PVariable Client::listClientServers(std::string id)
{
	try
	{
		std::vector<std::shared_ptr<RemoteRpcServer>> servers;
		{
			...
			for(std::map<int32_t, std::shared_ptr<RemoteRpcServer>>::const_iterator i = _servers.begin(); i != _servers.end(); ++i)
			{
				if(i->second->removed || (i->second->getServerClientInfo()->sendEventsToRpcServer && i->second->getServerClientInfo()->closed)) continue;
