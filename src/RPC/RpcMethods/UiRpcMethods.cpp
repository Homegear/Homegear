...
BaseLib::PVariable RpcAddUiElement::invoke(BaseLib::PRpcClientInfo clientInfo, BaseLib::PArray parameters) {
  try {
    ...
    if (!clientInfo || !clientInfo->acls->checkMethodAccess("addUiElement"))
    ...
    if (parameters->size() == 4) {
      ...
      auto metadata = (parameters->size()
                           == 3 ? parameters->at(2) : std::make_shared<BaseLib::Variable>(BaseLib::VariableType::tStruct));
...
BaseLib::PVariable RpcCheckUiElementSimpleCreation::invoke(BaseLib::PRpcClientInfo clientInfo,
  ...
  try {
    ...
    if (!clientInfo || !clientInfo->acls->checkMethodAccess("checkUiElementSimpleCreation"))
      return BaseLib::Variable::createError(-32603, "Unauthorized.");
...
BaseLib::PVariable RpcGetAllUiElements::invoke(BaseLib::PRpcClientInfo clientInfo, BaseLib::PArray parameters) {
  try {
    if (!clientInfo || !clientInfo->acls->checkMethodAccess("getAllUiElements"))
      return BaseLib::Variable::createError(-32603, "Unauthorized.");
...
BaseLib::PVariable RpcGetAvailableUiElements::invoke(BaseLib::PRpcClientInfo clientInfo, BaseLib::PArray parameters) {
  try {
    if (!clientInfo || !clientInfo->acls->checkMethodAccess("getAvailableUiElements"))
      return BaseLib::Variable::createError(-32603, "Unauthorized.");
...
BaseLib::PVariable RpcGetCategoryUiElements::invoke(BaseLib::PRpcClientInfo clientInfo, BaseLib::PArray parameters) {
  try {
    ...
    if (!clientInfo || !clientInfo->acls->checkMethodAccess("getCategoryUiElements"))
      return BaseLib::Variable::createError(-32603, "Unauthorized.");

    return GD::uiController->getUiElementsInCategory(clientInfo,
...
BaseLib::PVariable RpcGetRoomUiElements::invoke(BaseLib::PRpcClientInfo clientInfo, BaseLib::PArray parameters) {
  try {
    ...
    if (!clientInfo || !clientInfo->acls->checkMethodAccess("getRoomUiElements"))
      return BaseLib::Variable::createError(-32603,
                                            "Unauthorized.");

    return GD::uiController->getUiElementsInRoom(clientInfo,
                                                 (uint64_t)parameters->at(0)->integerValue64,
                                                 parameters->at(1)->stringValue);
  }
  catch (const std::exception &ex) {
...
BaseLib::PVariable RpcGetUiElementMetadata::invoke(BaseLib::PRpcClientInfo clientInfo, BaseLib::PArray parameters) {
  try {
    ...
    if (!clientInfo || !clientInfo->acls->checkMethodAccess("getUiElementMetadata"))
      return BaseLib::Variable::createError(-32603, "Unauthorized.");
...
BaseLib::PVariable RpcSetUiElementMetadata::invoke(BaseLib::PRpcClientInfo clientInfo, BaseLib::PArray parameters) {
  try {
    ...
    return GD::uiController->setUiElementMetadata(clientInfo,
                                                  (uint64_t)parameters->at(0)->integerValue64,
                                                  parameters->at(1));
  }
  catch (const std::exception &ex) {
