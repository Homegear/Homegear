  ...
  try {
    ...
    if (_printEvents) {
      ...
      for (uint32_t i = 0; i < parameters->at(3)->arrayValue->size(); ++i) {
        ...
        standardOutput(BaseLib::HelperFunctions::getTimeString() + ": ID >>" + (peerId > 999999 ? "0x" + BaseLib::HelperFunctions::getHexString(peerId, 8) : std::to_string(peerId)) + "<<, channel >>" + std::to_string(parameters->at(2)->integerValue)
                           + "<<, variable >>" + parameters->at(3)->arrayValue->at(i)->stringValue + "<<, source >>" + parameters->at(0)->stringValue + "<<, value >>" + value + "<<\n");
      }
    }
