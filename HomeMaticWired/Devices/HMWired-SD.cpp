...
namespace HMWired
{
HMWired_SD::HMWired_SD() : HMWiredDevice()
{
	init();
}

HMWired_SD::HMWired_SD(uint32_t deviceID, std::string serialNumber, int32_t address) : HMWiredDevice(deviceID, serialNumber, address)
{
	init();
}
...
void HMWired_SD::init()
{
	try
	{
		...
		_deviceType = GD::deviceTypes.get(DeviceID::HMWIREDSD);
	}
...
bool HMWired_SD::packetReceived(std::shared_ptr<Packet> packet)
{
	try
	{
